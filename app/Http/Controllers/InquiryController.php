    ...
    public function index(InquiryRequest $request) {
        ...
        if (Auth::user()->role_id <> 5) {
            ...
            if ((isset($data['inquiry_filter'])) && $data['inquiry_filter'] != '') {
               ...
               $sql =Inquiry::with('customer', 'delivery_location', 'inquiry_products.inquiry_product_details', 'inquiry_products.unit', 'createdby')->where('inquiry_status', 'pending')->orderBy('created_at', 'desc')->where('is_approved', '=', 'yes')->toSql();
               echo $sql;
