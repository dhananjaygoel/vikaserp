    ...
    <div class="col-lg-12">
        <div class="row">
            <div class="col-lg-12">
                ...
                <form method="get" action="{{URL::action('CustomerController@bulk_set_price')}}" id="filter_search">
                            ...
                            <div class="table-responsive">
                                <table id="table-example" class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <?php $product = $product_category; ?>
                                        <tr>                                            
                                            <td>{{$i++}}</td>
                                            <td>{{$c->owner_name}}</td>
                                            @foreach($product as $key1=>$prod)
                                            <td>
                                                <input type='text' name="set_diff[{{$key}}][{{$key1}}][price]" id='' value="" style="width: 40px;">
                                                <input type='hidden' name="set_diff[{{$key}}][{{$key1}}][cust_id]" id='' value="{{$c->id}}" style="width: 40px;">
                                                <input type='hidden' name="set_diff[{{$key}}][{{$key1}}][product_id]" id='' value="{{$prod->id}}" style="width: 40px;">
                                            </td>
                                            @endforeach
                                        </tr>
                                        @endforeach
